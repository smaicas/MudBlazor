...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class SelectTests : BunitTest
    {
        ...
        /// <summary>
        /// Test Label Class Parameter
        /// <summary>
        [Test]
        public async Task LabelParameter_Should_SetSelectLabelClassParameter()
        {
            // Arrange
            var comp = Context.RenderComponent<SelectLabelClassTest>();

            // Act
            var selectComp = comp.FindComponents<MudSelect<string>>()
                .FirstOrDefault(x => x.Instance.Label == "Test1");

            //Assert
            _ = selectComp.Instance.LabelClass.Should().Be("mud-select-input test-class");
        }

        [Test]
        public async Task LabelParameter_Should_HaveDefaultValue()
        {
            // Arrange
            var comp = Context.RenderComponent<SelectLabelClassTest>();
            ...
            // Act
            var selectComp = comp.FindComponents<MudSelect<string>>()
                .FirstOrDefault(x => x.Instance.Label == "Test2");

            //Assert
            _ = selectComp.Instance.LabelClass.Should().Be("mud-select-input");
        }
    }
}
